...
func (a *App) fetchFlag(flagName string) (*Flag, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("URL do flag-service invalida: %w", err)
	}

	apiKey := os.Getenv("SERVICE_API_KEY")
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, serviceURL, nil) // #nosec G704 -- URL validada e caminho escapado
	if err != nil {
		return nil, fmt.Errorf("erro ao criar requisicao para flag-service: %w", err)
	}
	req.Header.Set("Authorization", "Bearer "+apiKey)

	resp, err := a.HttpClient.Do(req) // #nosec G704 -- destino validado por buildServiceURL
	if err != nil {
...
func (a *App) fetchRule(flagName string) (*TargetingRule, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("URL do targeting-service invalida: %w", err)
	}
...
func getDeterministicBucket(input string) int {
	// SHA-256 gera uma distribuicao estavel sem usar um algoritmo obsoleto.
...
func buildServiceURL(baseURL, resource, identifier string) (string, error) {
	...
	if parsed.Host == "" || parsed.User != nil {
		return "", fmt.Errorf("host ausente ou credencial embutida")
	}
	if !regexp.MustCompile(`^[A-Za-z0-9._-]+$`).MatchString(identifier) {
		return "", fmt.Errorf("identificador possui caracteres invalidos")
	}

	parsed.Path = strings.TrimRight(parsed.Path, "/") + "/" + resource + "/" + identifier
	parsed.RawQuery = ""
	parsed.Fragment = ""
